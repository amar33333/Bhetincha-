...
class DraggableTable extends Component {
  ...
  mountEvents() {
    ...
    headers.forEach((header, i) => {
      ...
      header.ondrop = e => {
        ...
        console.log("Column Sorted after drag");
      };
    });

    const headersRow = Array.prototype.slice.call(
      document.querySelectorAll(".rt-tr-group")
    );

    headersRow.forEach((headerRow, i) => {
      headerRow.setAttribute("draggable", true);
      //the dragged header
      headerRow.ondragstart = e => {
        e.stopPropagation();
        this.dragged = i;
      };

      headerRow.ondrag = e => e.stopPropagation;

      headerRow.ondragend = e => {
        e.stopPropagation();
        setTimeout(() => (this.dragged = null), 1000);
      };

      //the dropped headerRow
      headerRow.ondragover = e => {
        e.preventDefault();
      };
      ...
      headerRow.ondrop = e => {
        e.preventDefault();
        const { target, dataTransfer } = e;
        this.reorderRow.push({ a: i, b: this.dragged });
        this.setState({ trigger: Math.random() });
        console.log("Rows Sorted after drag");
      };
    });
  }
  componentDidMount() {
  ...
  render() {
    ...
    this.reorderRow.forEach(o => rows.splice(o.a, 0, rows.splice(o.b, 1)[0]));
